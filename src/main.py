...
class QueryThread(QThread):
    ...
    def __init__(
        self, DATA: Data, nth_batch: int, app: "ParquetSQLApp", query: str = None
    ):
        super().__init__()
...
class ParquetSQLApp(QMainWindow):
    def __init__(self, file_path=None):
        ...
        self.rows_per_page = settings.render_vars(
            settings.result_pagination_rows_per_page
        )
    ...
    def initUI(self):
        ...
        self.browseButton.setStyleSheet(
            f"background-color: {settings.colour_browseButton}"
        )
        self.browseButton.clicked.connect(self.browseFile)
        layout.addWidget(self.browseButton)
        # View
        self.ViewFileButton = QPushButton("[2]View")
        self.ViewFileButton.setStyleSheet(
            f"background-color: {settings.colour_browseButton}"
        )
        self.ViewFileButton.clicked.connect(self.ViewFile)
        layout.addWidget(self.ViewFileButton)
        # SQL Edit
        self.sqlLabel = QLabel(
            f"Data Query - AS {settings.render_vars(settings.default_data_var_name)}:"
        )
        ...
        self.tableInfoButton.setStyleSheet(
            f"background-color: f{settings.colour_tableInfoButton}"
        )
        ...
        self.resultTable.setStyleSheet(
            f"background-color: f{settings.colour_resultTable}"
        )
    ...
    def ViewFile(self):
        if self.file_path:
            if not hasattr(self, "DATA"):
                self.DATA = Data(
                    path=file_path,
                    virtual_table_name=settings.render_vars(
                        settings.default_data_var_name
                    ),
                    batchsize=int(settings.result_pagination_rows_per_page),
                )
    ...
    def nextPage(self):
        ...
        if (
            isinstance(self.DATA.total_batches, int)
            and self.page >= self.DATA.total_batches
        ):
            return
        self.page += 1
        self.loadPage()
    ...
    def showContextMenu(self, pos):
        ...
        if column >= 0:
            ...
            copy_column_action.triggered.connect(
                lambda: self.copyColumnName(column_name)
            )
